...
// ENTER TIM3 FOR FreeRTOS !!!!!! CubeMX -> SYS
...
void vTaskCode_ONE( void * pvParameters )
{
    ...
    for( ;; )
    {
		...
		vTaskPrioritySet( NULL, tskIDLE_PRIORITY + count_one );
    }
...
void vTaskCode_TWO( void * pvParameters )
{
    ...
    for( ;; )
    {
		...
		// Use the handle to raise the priority of the created task.
		vTaskPrioritySet( NULL, tskIDLE_PRIORITY + count_two );
    }
...
// function is the idle task hook and it is called automatically=======================
void vApplicationIdleHook( void * pvParameters )
{
	static uint32_t previos_time = 0;
	if( HAL_GetTick() - previos_time > 1000 ){
		previos_time = HAL_GetTick();
